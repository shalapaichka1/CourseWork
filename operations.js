function renderOperationsData(categType) {
    ...
    .then(data => {
      for (let i = 0; i < data.length; i++) { // создание элементов операций
        if (true) {
          if (data[i].operationsUserId === parseInt(currentUserId) && data[i].categoryName === categType) {
            ...
            if(data[i].categoryType === 'expenses') {
  ...
  function addOperationsData(categoryName, descriptionInput, priceInput, categoryType) {
